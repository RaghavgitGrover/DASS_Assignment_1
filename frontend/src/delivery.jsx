...
const Delivery = () => {
    ...
    const handleOtpVerification = async (orderNumber, itemIndex, orderOtp) => {
        ...
        else toast.error("Incorrect OTP. Please try again.", { autoClose: 2000 });
    };
    ...
    const handleOtpChange = (index, value) => {
        setOtpInputs(prev => ({ ...prev, [index]: value }));
    };
                                            ...
                                            {order.items.map((item, index) =>
                                                item.seller === currentUser && item.status !== "Delivered" && (
                                                    <div key={index} className="mb-2">
                                                        ...
                                                        <span className={`badge ${getStatusColor(item.status)} p-2`}>
                                                            {item.status}
                                                        </span>
                                                        <br />
                                                        ...
                                                        {item.status !== "Delivered" && (
                                                            <div className="mt-2">
                                                                ...
                                                                <button
                                                                    ...
                                                                    onClick={() => handleOtpVerification(order.orderNumber, index, order.otp)}
                                                                >
                                                                    Check OTP
                                                                </button>
                                                            </div>
